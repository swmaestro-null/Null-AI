...
class Solver(object):
    ...
    def train(self):
        ...
        for e in range(start_epoch, self.epoch):
            ...
            for i in range(iterations):
                ...
                print("@@@@@",elastic_reference.size(),reference.size(),sketch.size())
                loss_dict = dict()
    ...
    def test(self):
        ...
        reference = Image.open(osp.join(test_img_dir, '{}_color.png'.format(2955068))).convert('RGB')
        sketch = Image.open(osp.join(test_img_dir, '{}_sketch.png'.format(2955070))).convert('L')
        ...
        with torch.no_grad():
            ...
            image_report.append(fake_result)
            x_concat = torch.cat(image_report, dim=3)
            sample_path = os.path.join(self.test_dir, '{}-images{}.jpg'.format(1, '_test'))
            save_image(self.denorm(x_concat.data.cpu()), sample_path, nrow=1, padding=0)
            print('Saved real and fake images into {}...'.format(self.sample_dir))
